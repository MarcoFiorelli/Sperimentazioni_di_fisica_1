...
files=['raggio1_non.txt', 'raggio2_non.txt', 'raggio3_non.txt', 'raggio4_non.txt', 'raggio5_non.txt', 'raggio6_non.txt', 'raggio7_non.txt', 'raggio8_non.txt']
...
with open('RAGGI_REALI.txt', 'w') as file:
    for i in range(8):
        file.write(f"{raggi[i]}\t{incertezze_raggi[i]}\n")
...
plt.savefig('fit_non_newtoniano.png')
plt.show()

print(velocità)

with open('velocità_non.txt', 'w') as file:
    for i in range(8):
...
with open('velocità_corrette_non.txt', 'w') as file:
    for i in range(8):
...
for i in range(8):
    eta=2/9*pow(raggi[i], 2)/velocità_corrette[i]*(7870-1032)*9.8106
    viscosità.append(eta)
for i in range (8):
    delta_eta=math.sqrt(pow(incertezze_raggi[i]*4/9*raggi[i]/velocità_corrette[i]*(7870-1032)*9.8106, 2)+
                        pow(velocità_corrette_incertezze[i]*2/9*pow(raggi[i]/velocità_corrette[i], 2)*(7870-1032)*9.8106, 2)+
...
for i in range(8):
    eta=2/9*pow(raggi[i], 2)/velocità[i]*(7870-1032)*9.8106
    viscosità_non_corrette.append(eta)
for i in range (8):
    delta_eta=math.sqrt(pow(incertezze_raggi[i]*4/9*raggi[i]/velocità[i]*(7870-1032)*9.8106, 2)+
                        pow(velocità_incertezze[i]*2/9*pow(raggi[i]/velocità[i], 2)*(7870-1032)*9.8106, 2)+
...
temperature=[]
temp=np.loadtxt('temperature_non.txt')
temp1=temp[:, 0]
temp2=temp[:, 1]
temp3=temp[:, 2]
for i in range (8):
